...
class RFFigure(Figure):
    ...
    def set_ylabels(self):
        self.axr.set_ylim(
            self.rfidx[self.ipage][0], self.rfidx[self.ipage][0] + self.maxidx + 1
        )
        self.axr.set_yticks(
            np.arange(
                self.rfidx[self.ipage][0], self.rfidx[self.ipage][0] + self.maxidx + 1
    ...
    def read_sac(self, dt=0.1, order="baz"):
        ...
        if len(glob.glob(join(self.rfpath, "*P_R.sac"))) != 0:
            ...
            raise FileNotFoundError("No valid RFs in {}".format(self.rfpath))
        self.log.RFlog.info("Reading PRFs from {}".format(self.rfpath))
        self.filenames = [
            basename(sac_file).split("_")[0] for sac_file in sorted(tmp_files)
        ]
        self.phases = [
            basename(sac_file).split("_")[1] for sac_file in sorted(tmp_files)
        ]
        self.rrf = (
            obspy.read(join(self.rfpath, "*_{}.sac".format(self.comp)))
            .sort(["starttime"])
            .resample(1 / dt, window=None)
        )
        self.trf = (
            obspy.read(join(self.rfpath, "*_T.sac"))
            .sort(["starttime"])
            .resample(1 / dt, window=None)
        )
        ...
        self.staname = (
            self.rrf[0].stats.network + "." + self.rrf[0].stats.station
        ).strip(".")
        # self.staname = basename(self.rfpath)
        self.fig.suptitle(
            "{} (Latitude: {:.2f}\N{DEGREE SIGN}, Longitude: {:.2f}\N{DEGREE SIGN})".format(
                self.staname, self.rrf[0].stats.sac.stla, self.rrf[0].stats.sac.stlo
            ),
            fontsize=20,
        )
    ...
    def plotwave(self):
        ...
        for i in range(self.evt_num):
            ...
            (self.rlines[i],) = self.axr.plot(
                self.time_axis, r_amp_axis, color="black", linewidth=0.2
            )
            (self.tlines[i],) = self.axt.plot(
                self.time_axis, t_amp_axis, color="black", linewidth=0.2
            )
            self.rwvfillpos[i] = self.axr.fill_between(
                self.time_axis,
                r_amp_axis,
                bound + i + 1,
                where=r_amp_axis > i + 1,
                facecolor="red",
                alpha=0.3,
            )
            self.rwvfillnag[i] = self.axr.fill_between(
                self.time_axis,
                r_amp_axis,
                bound + i + 1,
                where=r_amp_axis < i + 1,
            ...
            self.twvfillpos[i] = self.axt.fill_between(
                self.time_axis,
                t_amp_axis,
                bound + i + 1,
                where=t_amp_axis > i + 1,
                facecolor="red",
                alpha=0.3,
            )
            self.twvfillnag[i] = self.axt.fill_between(
                self.time_axis,
                t_amp_axis,
                bound + i + 1,
                where=t_amp_axis < i + 1,
    ...
    def plotbaz(self):
        ...
        self.axg.scatter(
            self.gcarc, np.arange(self.evt_num) + 1, color="#ff7f0e", alpha=0.6
        )
    ...
    def finish(self):
        ...
        with open(
            os.path.join(self.rfpath, self.staname + "finallist.dat"), "w+"
        ) as fid:
            for i in range(self.evt_num):
                if self.goodrf[i] == 0:
                    ...
                    fid.write(
                        "%s %s %6.3f %6.3f %6.3f %6.3f %6.3f %8.7f %6.3f %6.3f\n"
                        % (
                            self.filenames[i],
                            self.phases[i],
                            evla,
    ...
    def plot(self):
        ...
        stadata = RFStation.read_stream(
            newrfs,
            self.rayp[goodidx],
            self.baz[goodidx],
            prime_comp=self.comp,
            stream_t=newtrfs,
        )
