...
def read_catalog(
    ...
    eq_lst = eq_lst[
        ...
        & (eq_lst["mag"] >= magmin)
        & (eq_lst["mag"] <= magmax)
        & (eq_lst["evdp"] >= depthmin)
        & (eq_lst["evdp"] <= depthmax)
        & (dis >= dismin)
        & (dis <= dismax)
    ]
    return eq_lst
...
def fetch_waveform(eq_lst, para, model, logger):
    ...
    try:
        ...
        logger.RFlog.error(
            "Please load station information and search earthquake before fetch waveform"
        )
    ...
    for i, row in eq_lst.iterrows():
        ...
        arrivals = model.get_travel_times(
            row["evdp"], daz.delta, phase_list=[para.phase]
        )
        if not arrivals:
            logger.RFlog.error(
                "The phase of {} with source depth {} and distance {} is not exists".format(
        ...
        if len(arrivals) > 1:
            logger.RFlog.error(
                "More than one phase were calculated with source depth of {} and distance of {}".format(
                    row["evdp"], daz.delta
                )
            )
        else:
        ...
        try:
            logger.RFlog.info(
                "Fetch waveforms of ({}/{}) event {} from {}".format(
                    i + 1, eq_lst.shape[0], datestr, para.data_server
                )
            )
            st = query.client.get_waveforms(
                para.stainfo.network,
                para.stainfo.station,
                para.stainfo.location,
                para.stainfo.channel,
                t1,
                t2,
            )
            ...
            logger.RFlog.error(
                "Error in fetching waveforms of event {}: {}".format(
                    datestr, str(e).strip()
                )
            )
            continue
        ...
        this_df = pd.DataFrame(
            [[daz.delta, daz.baz, this_eq, datestr]], columns=new_col, index=[i]
        )
...
def match_eq(
    ...
    for ref_sac in ref_eqs:
        ...
        if isinstance(offset, (int, float)):
            sac_files.append(
                [datestr, UTCDateTime.strptime(datestr, dateformat), -offset]
            )
        elif offset is None:
            try:
    ...
    for datestr, b_time, offs in sac_files:
        ...
        results = eq_lst[
            (eq_lst.date > date_range_begin) & (eq_lst.date < date_range_end)
        ]
        if len(results) != 1:
            continue
        try:
        ...
        this_df = pd.DataFrame(
            [[daz.delta, daz.baz, this_eq, datestr]],
            columns=new_col,
...
class RF(object):
    def __init__(self, cfg_file=None, log=None):
        ...
        if cfg_file is None:
            ...
            raise TypeError(
                "cfg should be in 'str' format rather than '{0}'".format(type(cfg_file))
            )
        ...
        try:
            if self.para.use_remote_data:
                self.logger.RFlog.info(
                    "Load station info of {}.{} from {} web-service".format(
                        self.para.stainfo.network,
                        self.para.stainfo.station,
                ...
                if use_date_range:
                    self.para.stainfo.get_station_from_ws(
                        starttime=self.para.date_begin, endtime=self.para.date_end
                    )
                else:
                    self.para.stainfo.get_station_from_ws()
                try:
                ...
                self.logger.RFlog.info(
                    "Load station info from {0}".format(self.para.datapath)
                )
                self.para.stainfo.load_stainfo(
                    self.para.datapath, self.para.ref_comp, self.para.suffix
                )
            self.logger.RFlog.info(
                "{}.{}, latitude: {:.3f}, longitude: {:.3f}".format(
                    self.para.stainfo.network,
                    self.para.stainfo.station,
                    self.stainfo.stla,
                    self.stainfo.stlo,
    ...
    def search_eq(self, local=False, catalog=None):
        if not local:
            try:
                self.logger.RFlog.info(
                    "Searching earthquakes from {}".format(self.para.cata_server)
                )
                ...
                query.get_events(
                    ...
                    latitude=self.para.stainfo.stla,
                    longitude=self.para.stainfo.stlo,
                    minmagnitude=self.para.magmin,
                    maxmagnitude=self.para.magmax,
                    minradius=self.para.dismin,
                    maxradius=self.para.dismax,
                    mindepth=self.para.depthmin,
                    maxdepth=self.para.depthmax,
                    catalog=catalog,
                )
            ...
            try:
                self.logger.RFlog.info(
                    "Searching earthquakes from {0} to {1}".format(
                        self.date_begin.strftime("%Y.%m.%dT%H:%M:%S"),
                        self.date_end.strftime("%Y.%m.%dT%H:%M:%S"),
                    )
                )
                self.eq_lst = read_catalog(
                    self.para.catalogpath,
                    self.para.date_begin,
                    self.para.date_end,
                    self.para.stainfo.stla,
                    self.para.stainfo.stlo,
                    magmin=self.para.magmin,
                    magmax=self.para.magmax,
                    dismin=self.para.dismin,
                    dismax=self.para.dismax,
                    depthmin=self.para.depthmin,
                    depthmax=self.para.depthmax,
                )
            except Exception as e:
    ...
    def match_eq(self):
        ...
        try:
            if self.para.use_remote_data:
                self.logger.RFlog.info(
                    "Fetch seismic data from {}".format(self.para.data_server)
                )
                self.eqs = fetch_waveform(
                    self.eq_lst, self.para, self.model, self.logger
                )
            else:
                ...
                self.eqs = match_eq(
                    ...
                    self.para.stainfo.stlo,
                    self.logger,
                    ref_comp=self.para.ref_comp,
                    suffix=self.para.suffix,
                    offset=self.para.offset,
                    tolerance=self.para.tolerance,
        ...
        if self.eqs.shape[0] == 0:
            self.logger.RFlog.warning(
                "No earthquakes associated, please check configurations.".format(
                    self.eqs.shape[0]
                )
            )
            sys.exit(1)
            ...
            self.logger.RFlog.info(
                "{0} earthquakes are associated".format(self.eqs.shape[0])
            )
    ...
    def loadpjt(cls, path):
        ...
        if not exists(pjt.para.datapath):
            pjt.logger.RFlog.error(
                "Data path {} was not found".format(pjt.para.datapath)
            )
            sys.exit(1)
        pjt.load_stainfo()
        pjt.eqs = rfdata["eqs"]
        for _, row in pjt.eqs.iterrows():
            try:
                ...
                pjt.logger.RFlog.warning(
                    "Cannot read {}, skipping".format(row["data"].filestr)
                )
                continue
        return pjt
    ...
    def detrend(self):
        ...
        for i, row in self.eqs.iterrows():
            try:
                ...
                self.logger.RFlog.error(
                    "Data error in {}".format(row["date"].strftime("%Y.%j.%H.%M.%S"))
                )
                drop_idx.append(i)
    ...
    def filter(self, freqmin=None, freqmax=None, order=4):
        ...
        self.logger.RFlog.info(
            "Filter all data from {0} to {1}".format(freqmin, freqmax)
        )
        for _, row in self.eqs.iterrows():
    ...
    def cal_phase(self):
        self.logger.RFlog.info(
            "Calculate {} arrivals and ray parameters for all data".format(
                self.para.phase
            )
        )
        for _, row in self.eqs.iterrows():
            row["data"].get_arrival(
                self.model, row["evdp"], row["dis"], phase=self.para.phase
            )

    def baz_correct(self, time_b=10, time_e=20, offset=90, correct_angle=None):
        if correct_angle is not None:
            self.logger.RFlog.info(
                "correct back-azimuth with {} deg.".format(correct_angle)
            )
            ...
            for i, row in self.eqs.iterrows():
                curr_baz, ampt = row["data"].search_baz(
                    row["bazi"], time_b=time_b, time_e=time_e, offset=offset
                )
            ...
            if None in shift_all:
                ...
                sys.exit(1)
            self.baz_shift = np.mean(
                shift_all[np.where(np.logical_not(np.isnan(shift_all)))]
            )
            self.logger.RFlog.info(
                "Average {:.1f} deg offset in back-azimuth".format(self.baz_shift)
            )
    ...
    def rotate(self, search_inc=False):
        ...
        self.logger.RFlog.info(
            "Rotate {0} phase to {1}".format(self.para.phase, method)
        )
        drop_idx = []
        for i, row in self.eqs.iterrows():
            try:
            ...
            if search_inc:
                self.logger.RFlog.info(
                    "The incidence angle of {} was corrected by {:.1f} deg".format(
    ...
    def drop_eq_snr(self, length=None, z_only=False):
        ...
        self.logger.RFlog.info(
            "Reject data record with SNR less than {0}".format(self.para.noisegate)
        )
        drop_lst = []
        for i, row in self.eqs.iterrows():
        ...
        self.logger.RFlog.info(
            "{0} events left after SNR calculation".format(self.eqs.shape[0])
        )

    def trim(self):
        self.logger.RFlog.info(
            "Trim waveforms from {0:.2f} before {2} to {1:.2f} after {2}".format(
    ...
    def pick(self, prepick=True, stl=5, ltl=10, pickdir=None):
        ...
        if pickdir:
            ...
            for _, row in self.eqs.iterrows():
                ...
                if row["data"].datestr not in picks:
                    ...
                    row["data"].phase_trigger(
                        self.para.time_before,
                        self.para.time_after,
                        prepick=prepick,
                        stl=stl,
                        ltl=ltl,
                    )
                ...
                row["data"].phase_trigger(
                    ...
                    stl=stl,
                    ltl=ltl,
                    custom_shift=picks[row["data"].datestr][1],
                )
        elif prepick:
            for _, row in self.eqs.iterrows():
                self.logger.RFlog.info(
                    "Pre-pick {} arrival using STA/LTA method".format(self.para.phase)
                )
                row["data"].phase_trigger(
                    self.para.time_before,
                    self.para.time_after,
        ...
        self.logger.RFlog.info(
            "{0} events left after visual checking".format(self.eqs.shape[0])
        )
    ...
    def deconv(self):
        ...
        for i, row in self.eqs.iterrows():
            count += 1
            try:
                row["data"].deconvolute(
                    shift,
                    time_after,
                    method=self.para.decon_method,
                    f0=self.para.gauss,
                    only_r=self.para.only_r,
                    itmax=self.para.itmax,
                    minderr=self.para.minderr,
                    wlevel=self.para.wlevel,
                    target_dt=self.para.target_dt,
                )
                if self.para.decon_method == "iter":
                    self.logger.RFlog.info(
                        "Iterative Decon {0} ({3}/{4}) iterations: {1}; final RMS: {2:.4f}".format(
                            row["data"].datestr,
                            row["data"].rf[0].stats.iter,
                            row["data"].rf[0].stats.rms[-1],
                            count,
                            self.eqs.shape[0],
                        )
                    )
                    ...
                    self.logger.RFlog.info(
                        "Water level Decon {} ({}/{}); RMS: {:.4f}".format(
                            row["data"].datestr,
                            count,
                            self.eqs.shape[0],
                            row["data"].rf[0].stats.rms,
                        )
                    )
            except Exception as e:
                self.logger.RFlog.error("{}: {}".format(row["data"].datestr, e))
    ...
    def saverf(self, gauss=None):
        npts = int(
            (self.para.time_before + self.para.time_after) / self.para.target_dt + 1
        )
        ...
        if scalar_instance(self.para.gauss):
            ...
            if gauss in self.para.gauss:
                ...
                raise ValueError(
                    "gauss should be a element in the seispy.para.RFPara.gauss"
                )

        if self.para.rmsgate is not None:
            self.logger.RFlog.info(
                "Save RFs with final RMS less than {:.2f} and criterion of {}".format(
                    self.para.rmsgate, self.para.criterion
                )
            )
        else:
            self.logger.RFlog.info(
                "Save RFs with and criterion of {}".format(self.para.criterion)
            )
        for i, row in self.eqs.iterrows():
            if row["data"].judge_rf(
                gauss,
                shift,
                npts,
                criterion=self.para.criterion,
                rmsgate=self.para.rmsgate,
            ):
                row["data"].saverf(
                    self.para.rfpath,
                    evtstr=row["date"].strftime("%Y.%j.%H.%M.%S"),
                    shift=shift,
                    evla=row["evla"],
                    evlo=row["evlo"],
                    evdp=row["evdp"],
