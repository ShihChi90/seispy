...
def rfharmo():
    ...
    parser.add_argument(
        "-e",
        help="Enlarge factor for plotting, defaults to 2",
        metavar="enf",
        default=2.0,
        type=float,
    )
    parser.add_argument(
        "-k",
        help="Suppress stacking RFs by back-azimuth, defaults to False",
        default=True,
        action="store_false",
    )
    parser.add_argument(
        "-o",
        help="Specify output path for saving constant component.",
        metavar="outpath",
        default=None,
        type=str,
    )
    parser.add_argument(
        "-p",
        help="Figure output path, defaults to ./",
        metavar="figure_path",
        default="./",
        type=str,
    )
    parser.add_argument(
        "-s",
        help="Resample RFs with sampling interval of dt",
        metavar="dt",
        default=None,
        type=float,
    )
    parser.add_argument(
        "-t",
        help="Time window from tb to te for triming RFs, NOTE: do not insert space before this argument, defaults to -2/10",
        metavar="tb/te",
        default="-2/10",
    )
...
def rfani():
    parser = argparse.ArgumentParser(
        description="Estimate crustal anisotropy with a Joint inversion method. See Liu and Niu (2012) in detail."
    )
    parser.add_argument("rfpath", type=str, help="Path to PRFs")
    parser.add_argument(
        "-t",
        help="Time window for searching Pms from tb to te",
        metavar="tb/te",
        required=True,
    )
    parser.add_argument(
        "-c",
        help="List file in text format for saving results, defaults to ./rfani.dat",
        default="rfani.dat",
        metavar="list_file_name",
    )
    parser.add_argument(
        "-l",
        help="Half length of time window cut around Pms phase, defaults to 3s",
        default=3,
        metavar="half_time_length",
        type=float,
    )
    parser.add_argument(
        "-r",
        help="Ray-parameter for moveout correction, defaults to 0.06 s/km",
        default=0.06,
        metavar="rayp",
    )
    parser.add_argument(
        "-m",
        help="velocity model for moveout correction. 'iasp91', 'prem'"
        ...
        default="iasp91",
        metavar="velocity_model",
    )
    parser.add_argument(
        "-o",
        dest="outpath",
        help="Directory to the image, defaults to current directory.",
        default="./",
    )
    parser.add_argument(
        "-p",
        help="If plot RFs stacked by back-azimuth, defaults to 'False'",
        dest="isplot",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-w",
        help="Weights of 3 anisotropic methods (order by R cosine energy, R cross-correlation and T energy), defaults to 0.4/0.4/0.2",
        dest="weight",
        default="0.4/0.4/0.2",
        metavar="w1/w2/w3",
    )
    ...
    try:
        ...
        if bf.size == 0 or bt.size == 0:
            raise ValueError(
                "No valid anisotropy peak found - likely insufficient back-azimuth coverage or poor data quality"
            )
        if bf.size == 1:
        ...
        print(f"Warning: Skipping station {rfsta.staname} - {e}")
        return
...
def ccp3d():
    ...
    parser = argparse.ArgumentParser(
        description="3-D CCP stacking with spaced grid bins."
    )
    parser.add_argument("cfg_file", type=str, help="Path to CCP configure file")
    parser.add_argument(
        "-s",
        help="Range for searching depth of D410 and D660, The results would be saved to 'peakfile' in cfg_file",
        metavar="d410min/d410max/d660min/d660max",
        default=None,
    )
    arg = parser.parse_args()
...
def ccp_profile():
    ...
    parser.add_argument("cfg_file", type=str, help="Path to CCP configure file")
    parser.add_argument(
        "-t", help="Output as a text file", dest="isdat", action="store_true"
    )
    arg = parser.parse_args()
    if arg.isdat:
        typ = "dat"
...
def get_pierce_points():
    ...
    parser.add_argument(
        "-o",
        help="filename of output file, defaults to ./pierce_points.dat",
        default="./pierce_points.dat",
        metavar="filename",
    )
    arg = parser.parse_args()
...
def common_parser():
    ...
    parser.add_argument(
        "-l",
        help="use local catalog, defaults to false",
        dest="islocal",
        action="store_true",
    )
    parser.add_argument(
        "-p",
        help="Wether or not manually pick arrival time and waveforms arround P/S phase with a GUI.",
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-r",
        help="Reverse components: N, E or NE",
        dest="comp",
        metavar="N|E|NE",
        default=None,
        type=str,
    ...
    parser.add_argument(
        "-s",
        help="Switch the East and North components",
        dest="isswitch",
        action="store_true",
    )
...
def prf():
    ...
    parser.add_argument(
        "-f",
        help="Specify finallist for re-calculating RFs and -l is invalid in this pattern",
        metavar="finallist",
        default=None,
    )
    arg = parser.parse_args()
...
def srf():
    parser = common_parser()
    parser.add_argument(
        "-i", help="Wether grid search incidence angle", action="store_true"
    )
    arg = parser.parse_args()
    pjt = RF(cfg_file=arg.cfg_file)
...
def plot_rt():
    ...
    parser.add_argument(
        "-c",
        help="prime component to plot, defaults to 'R'",
        default="R",
        type=str,
        metavar="[R|Q]",
    )
    ...
    parser.add_argument(
        "-k",
        help="The key to sort PRFs, avialible for 'event', 'evla', 'evlo', 'evdp',"
    ...
    parser.add_argument(
        "-t",
        help="Specify figure format. f = '.pdf', g = '.png', defaults to 'g'",
        dest="format",
        default="g",
        type=str,
        metavar="f|g",
...
def plot_r():
    ...
    parser.add_argument(
        "rfpath", help="Path to PRFs with a 'finallist.dat' in it", type=str
    )
    parser.add_argument(
        "-c",
        help="prime component to plot, defaults to 'R'",
    ...
    rfsta = RFStation(arg.rfpath, prime_comp=arg.c)
    rfsta.plotr(
        arg.output, enf=arg.enf, key=arg.k, xlim=[-2, arg.x], outformat=arg.format
    )
...
def get_events():
    ...
    parser = argparse.ArgumentParser(
        description="Get seismic events from IRIS Web-Service"
    )
    parser.add_argument(
        "-b",
        help="Start time, e.g., 20210101, 20210101020304",
        metavar="datetime",
        default=None,
    )
    parser.add_argument(
        "-c", help="Catalog type", metavar="GCMT|NEIC PDE|ISC", default=None
    )
    parser.add_argument(
        "-d",
        help="Radial geographic constraints with center point and distance range",
        metavar="<lat>/<lon>/<minradius>/<maxradius>",
        default=None,
    )
    parser.add_argument(
        "-e",
        help="End time, e.g., 20210101, 20210101020304",
        metavar="datetime",
        default=None,
    )
    parser.add_argument(
        "-m",
        help="Magnitude range, optional for max magnitude",
        metavar="<minmagnitude>[/<maxmagnitude>]",
        default=None,
    )
    parser.add_argument(
        "-r",
        help="Box range with min and max latitude and logitude, omited when specify '-d' ",
        metavar="<lat>/<lon>/<minradius>/<maxradius>",
        default=None,
    )
    parser.add_argument(
        "-p",
        help="Focal depth, optional for max depth",
        metavar="<mindepth>[/<maxdepth>]",
        default=None,
    )
    arg = parser.parse_args()
    args = {}
...
def get_stations():
    ...
    parser.add_argument(
        "-S", help="Server name, defaults to IRIS", metavar="server", default="IRIS"
    )
    parser.add_argument(
        "-b",
        help="Start time, e.g., 20210101, 20210101020304",
    ...
    parser.add_argument(
        "-c",
        help="Channel, wildcard is available like *,?,[EB]...",
        metavar="channel",
        default=None,
    )
    parser.add_argument(
